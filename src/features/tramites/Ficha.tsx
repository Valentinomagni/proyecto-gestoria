...
import { ArrowLeft, Ban, MessageSquare, PauseCircle, Plus } from "lucide-react";
...
export function Ficha({ id, alVolver }: { id: string; alVolver: () => void }) {
   ...
   *
   * No es un detalle de comodidad: la base tiene un `check` que exige el motivo cuando el
   * estado es `anulado` o `frenado_por_saldo`. Si se guardaran en dos pasos, el primero
   * fallaria — y con razon, porque entre uno y otro habria un instante con un tramite anulado
   * sin ningun motivo escrito.
   */
  const salir = useGuardar(
    ...
    {
      exito: "Trámite actualizado",
      invalidar: ["tramite", "tramites", "saldos", "tramite_eventos"],
    },
  );
